...
class Control():
    ...
    def learning(self):
        try:
            ...
            self.limit_upper_grad = input("ガウス分割と複製を行う上限値(標準0.005):") or self.grad_delta_upper_limit if self._mode != True else self.grad_delta_upper_limit
            ...
            self.dense_percent = input("???を指定してください(標準0.1):") or self.grad_delta_upper_limit if self._mode != True else self.grad_delta_upper_limit
...
class Load_colmap_data():
    ...
    def qvec_to_rotmat_torch(self, qvec: torch.Tensor) -> torch.Tensor:
        """
        qvec: (4,) tensor [qw, qx, qy, qz]
