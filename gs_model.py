...
from sh_utility import eval_sh
...
class _GS_model(torch.nn.Module):
    ...
    #optimizerの変更
    def changing_optimizer(self): #lrはテンソル
        param_lr_list = []
        for (name,param) in iter(self.named_parameters(recurse=False)):
            param_lr_list.append({"params":param,"lr":torch.tensor(self.lr[name],dtype=torch.float32,device="cuda")})
...
class _GS_model_with_ultra_param(_GS_model):
    def __init__(self, grad_delta_upper_limit,grad_threshold,percent_dense,variance_pixel_tile_max_width, lr=0.01, caller=None):
        ...
        #パラメータ学習率
        self.lr = {"grad_delta_upper_limit":lr,"grad_threshold":lr,"percent_dense":lr,"variance_pixel_tile_max_width":lr}
        self.changing_optimizer()
        #最初は勾配計算されないように設定
        self.changing_required_grad(False)
...
class _GS_model_with_super_param(_GS_model):
    def __init__(self,
                 ...
                 feature_lr,opacity_lr,
                 scaling_lr,rotation_lr,
                 prunning_min_opacity,
                 lr=0.01, caller=None):
        #GS_model_with_paramのみ利用しないとエラーが出る.
        super().__init__(caller)
        #self.mean_lr = torch.nn.Parameter(mean_lr)
        #self.others_lr = torch.nn.Parameter(others_lr)
        self.position_lr_init = torch.nn.Parameter(torch.tensor(position_lr_init,device="cuda",dtype=torch.float32))
        self.position_lr_final = torch.nn.Parameter(torch.tensor(position_lr_final,device="cuda",dtype=torch.float32))
        self.position_lr_delay_mult = torch.nn.Parameter(torch.tensor(position_lr_delay_mult,device="cuda",dtype=torch.float32))
        self.position_lr_max_steps = torch.nn.Parameter(torch.tensor(position_lr_max_steps,device="cuda",dtype=torch.float32))
        self.feature_lr = torch.nn.Parameter(torch.tensor(feature_lr,device="cuda",dtype=torch.float32))
        self.opacity_lr = torch.nn.Parameter(torch.tensor(opacity_lr,device="cuda",dtype=torch.float32))
        self.scaling_lr = torch.nn.Parameter(torch.tensor(scaling_lr,device="cuda",dtype=torch.float32))
        self.rotation_lr = torch.nn.Parameter(torch.tensor(rotation_lr,device="cuda",dtype=torch.float32))
        self.prunning_min_opacity = torch.nn.Parameter(torch.tensor(prunning_min_opacity,device="cuda",dtype=torch.float32))
        #パラメータ学習率
        self.lr = {"position_lr_init":lr,"position_lr_final":lr,"position_lr_delay_mult":lr,"position_lr_max_steps":lr,"feature_lr":lr,
                   "opacity_lr":lr,"scaling_lr":lr,"rotation_lr":lr,"prunning_min_opacity":lr}
        self.changing_optimizer()
        #最初は勾配計算されないように設定
        self.changing_required_grad(False)
...
class GS_model_with_param(_GS_model):
    ...
    def __init__(self, 
                 mean, variance_q, variance_scale, opacity, 
                 grad_delta_upper_limit,grad_threshold,
                 percent_dense,prunning_min_opacity,variance_pixel_tile_max_width,
                 position_lr_init,
                 position_lr_final,
                 position_lr_delay_mult,
                 position_lr_max_steps,
        ...
        self.super = _GS_model_with_super_param(
                 position_lr_init,
                 position_lr_final,
                 position_lr_delay_mult,
                 position_lr_max_steps,
                 feature_lr,opacity_lr,
                 scaling_lr,rotation_lr,
                 prunning_min_opacity,caller=self)
        ...
        #パラメータ学習率
        self.mean_lr_setfunc = Utilities.get_expon_lr_func(self.super.position_lr_init, self.super.position_lr_final,
                                                           lr_delay_steps=0, lr_delay_mult=self.super.position_lr_delay_mult,
                                                           max_steps=self.super.position_lr_max_steps)
        self.lr = {"mean":self.mean_lr_setfunc(0),"variance_q":self.super.rotation_lr,
                   "variance_scale":self.super.scaling_lr,"opacity":self.super.opacity_lr,"color":self.super.feature_lr}
        self.changing_optimizer()
        self._L_max = L_max
        #パラメーターとして使用しないローカル変数
        self.mean_grads_norm = torch.zeros(self.mean.shape[0],device="cuda",dtype=torch.float32)
        self.mean_grads_iter = torch.zeros(self.mean.shape[0],device="cuda",dtype=torch.int16)
        #勾配計算されないように設定
    ...
    def densify_and_split(self, scene_extent, N=2):
        # Extract points that satisfy the gradient condition
        scale = torch.exp(self.variance_scale)
        selected_pts_mask = torch.where(self.param_grads_per_iter_norm() >= self.ultra.grad_threshold, True, False)
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(scale, dim=1).values > self.ultra.percent_dense*scene_extent)
        stds = scale[selected_pts_mask].repeat(N,1)
        means =torch.zeros((stds.size(0), 3),device="cuda")
        samples = torch.normal(mean=means, std=stds)
        q = self.variance_q[selected_pts_mask] / torch.norm(self.variance_q[selected_pts_mask],dim=1,keepdim=True).clamp_min(1e-8)
        rots = Utilities.qvec_to_rotmat_torch_batch(q).repeat(N,1,1)
        new_mean = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self.mean[selected_pts_mask].repeat(N, 1)
        new_scaling = torch.log(scale[selected_pts_mask].repeat(N,1) / (0.8*N))
        new_rotation = self.variance_q[selected_pts_mask].repeat(N,1)
        new_opacity = self.opacity[selected_pts_mask].repeat(N,1)
        new_color = self.color[selected_pts_mask].repeat(N,1,1)
        new_mean_grads_norm = self.mean_grads_norm[selected_pts_mask].repeat(N)
        new_mean_grads_iter = self.mean_grads_iter[selected_pts_mask].repeat(N)
        
        #ガウス削除と追加を同時に行う.
        self.mean_grads_norm = torch.hstack((self.mean_grads_norm[selected_pts_mask == False],new_mean_grads_norm))
        self.mean_grads_iter = torch.hstack((self.mean_grads_iter[selected_pts_mask == False],new_mean_grads_iter))
        self.mean = torch.nn.Parameter(torch.vstack((self.mean[selected_pts_mask == False],new_mean)))
        self.variance_q = torch.nn.Parameter(torch.vstack((self.variance_q[selected_pts_mask == False],new_rotation)))
        self.variance_scale = torch.nn.Parameter(torch.vstack((self.variance_scale[selected_pts_mask == False],new_scaling)))
        self.opacity = torch.nn.Parameter(torch.vstack((self.opacity[selected_pts_mask == False],new_opacity)))
        self.color = torch.nn.Parameter(torch.vstack((self.color[selected_pts_mask == False],new_color)))
    ...
    def densify_and_prune(self, extent, lr):
        ...
        print("densify and pruningeしました")
        
    def reset_opacity(self,reset_opacity):
        opacities_new = torch.logit(torch.min(torch.sigmoid(self.opacity), torch.ones_like(self.opacity)*reset_opacity))
        print("opacityをリセットしました") #(self.opacity != opacities_new).sum()}個/{self.opacity.size(0)}
        self.opacity = torch.nn.Parameter(opacities_new)
        self.changing_optimizer()
    ...
    def forward(self, P, K, wh, image_sample):#input = 入力, super_param = 超パラメータ
        ...
        #色L(d)を計算
        L_d = eval_sh(self._L_max,
                      torch.transpose(self.color[None,:,:,:].repeat(shape_image,1,1,1),2,3),
                      -mean_camera / torch.norm(mean_camera, dim=-1, keepdim=True).clamp_min(1e-8)
                      )
        # Utilities.gpu_mem()
        ...
        z_inverse_index = torch.argsort(z_index)
    ...
    # )
        ...
        Utilities.gpu_mem()
        pixel_image_batch =[]
        #ガウシアンのgrad積算回数に可算するかどうかを決めるbooleanテンソル
        grad_iter = torch.zeros(shape_gausian,device="cuda",dtype=torch.bool).detach()
        for batch_i in range(shape_image):
            ...
            grad_iter |=  (boxsize_mask & z_index_minus_mask)[z_inverse_index[batch_i,:]]
            ...
            gausian_batch = torch.cumsum(Utilities.split_by_cumsum_parallel(gausian_boxsize_batch/1024, (1024**3*6/12)/1024),dim=0)
            # Utilities.gpu_mem()
        ...
        return [pixel_image_batch,image_sample,grad_iter]
